...
function computeConfidence() {
  ...
  state.confidence = { band, delta, top: top?.id || null, runnerUp: runner?.id || null, label };
}
...
function buildFocusStatement(primaryPath) {
  ...
  let descriptor = "";
  if (primaryPath === "levelUp") descriptor = maps.levelUp?.byLevelUpType?.[v.levelUpType] || "greater responsibility and impact";
  if (primaryPath === "thrive") descriptor = maps.thrive?.byThriveFocus?.[v.thriveFocus] || "my effectiveness and impact";
  if (primaryPath === "moveAcross") descriptor = maps.moveAcross?.byAcrossPurpose?.[v.acrossPurpose] || "broader perspective";
  if (primaryPath === "expandView") descriptor = maps.expandView?.byExploreMode?.[v.exploreMode] || "a short-term experience";
  if (primaryPath === "reset") descriptor = maps.reset?.byResetDriver?.[v.resetDriver] || "sustainability and clarity";
...
function renderIntro() {
  ...
  $app.innerHTML = `
    ...
    <p class="subcopy">
      Answer a few questions, and the system will suggest a path, explain why, and translate it into something a People Leader can coach against.
    </p>
    <div class="controls">
...
function finishDiagnostic() {
  ...
  state.phase = "diagnosticComplete";
...
function renderRecommendation() {
  ...
  $app.innerHTML = `
    ...
    <div class="controls" style="margin-top:14px">
      <button id="chooseDifferentBtn">Choose a different path</button>
      <button class="primary" id="exploreBtn">Explore this path</button>
    </div>
  `;
...
function renderChooseDifferent() {
  ...
  $app.innerHTML = `
    ...
    <div class="controls"><button id="backBtn">Back</button></div>
  `;
...
function renderPlan() {
  ...
  $app.innerHTML = `
    ...
    <p class="subcopy">These are practical moves that create evidence.</p>
    ...
    <div class="grid2">
      ...
      <div>
        ...
        <div class="summaryBox">${escapeHtml(selectedList)}</div>
      </div>
    </div>
  ...
  document.getElementById("backBtn").onclick = () => startRefinement(state.pathIntent.primaryPath);
...
function generateSummaries() {
  ...
  const leaderAsk = translation.leaderAsk || "Agree on what support would matter most.";
...
Next steps to test and build
${experimentBullets}

What to discuss with your People Leader
${state.openQuestion}
`;

  const leader =
`People Leader Conversation Brief
...
function renderSummary() {
  ...
  $app.innerHTML = `
    ...
    <div class="summaryBox">${escapeHtml(tab==="employee" ? employee : leader)}</div>
...
async function boot() {
  state.phase = "loading";
  $app.innerHTML = `<div class="screenTitle">Loading</div><h2 class="prompt">Loading configuration…</h2>`;
...
boot().catch(err => {
  console.error(err);
  $app.innerHTML = `<div class="screenTitle">Error</div><h2 class="prompt">Could not load config.</h2>`;
});
